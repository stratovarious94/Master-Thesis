...
class BarcodeEncoder:
    """
    Uses the models trained in Categorizing the barcoded product id
    """
    ...
    def encode(self):
        ...
        train = pd.read_csv(project_source + '/data/matching/train/train_' + str(self.min_el) +
                            '.csv')[['id', self.task]].rename(columns={self.task: 'label'})
        ...
        res = pd.DataFrame({'embedding': list(embeddings), 'id': train['label']})
        ...
        res = pd.DataFrame({'embedding': list(embeddings), 'id': valid['label']})
        ...
        res = pd.DataFrame({'embedding': list(embeddings), 'id': test['label']})
